...
public class GiveRankRewardsCommand extends BaseCommand implements Listener {
    ...
    public void onCommand(CommandArgs commandArgs) {
        ...
        Profile profile = getProfileManager().getProfile(args[2]);
        ...
        if (args[0].equalsIgnoreCase("check")) {
            ...
            if (player != null) {
                for (RankObject rankObject : rankObjects)
                    if (rankObject.getRankName().equalsIgnoreCase(LuckPerms.getApi().getUser(player.getUniqueId()).getPrimaryGroup()))
    ...
    public void load() {
        if (getCarbyne().getConfig().getStringList("RankRewardCommands").size() > 0) {
            for (String arg : getCarbyne().getConfig().getStringList("RankRewardCommands")) {
